...
_.extend(Updater.prototype, {
  cachePath: path.resolve(__dirname, '../data/cache.js'),
  cacheLockPath: path.resolve(__dirname, '../data/cache.js.lock'),
  iniFile: path.resolve(__dirname, '../data/udgerdata_old.ini'),
  ...
  initialize: function(options, callback) {
    console.info('uas-parser initialize called');
    this.updateInterval = options.updateInterval;
    this.useLocalFile = options.useLocalFile;
    ...
    lockFile.lock(this.cacheLockPath, { stale: 60 * 1000 }, function(error) {
      if(error) {
        console.info('uas-parser: failed to lock file');
        this.finish(null, cache);
        return true;
      }

      console.info(`uas-parser: calling fs.stat on cachePath:${this.cachePath}`);
  ...
  handleFileStat: function(error, stats) {
    ...
    if(this.useLocalFile){
      console.info(`uas-parser: using local udger ini file to update cache:${this.iniFile}`);
      this.loadDataFromFile.call(this);
      this.finish(null, cache);
    }
    else if(stats.mtime.getTime() < new Date().getTime() - this.updateInterval) {
      console.info(`uas-parser: calling checkVersion in handleFileStat`);
      this.checkVersion();
    } 
    else {
      console.info(`uas-parser: calling finish. no update needed`);
      this.finish(null, cache);
    }
  },
  ...
  loadDataFromFile: function(){
    console.info('[uas-parser] Loading data from file:'+this.iniFile);
    fs.readFile(this.iniFile, 'utf8', function(err, data){
      if(err) { 
      }
      else{
        this.parseIni(data);
  ...
  downlodChecksum: function(callback) {
